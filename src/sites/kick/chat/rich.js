...
// Builds the card for a plucked rich token (see chat.pluckRichContent)
// and starts loading its data.
export function renderRichCard(chat, rich) {
	const body = createElement('div', {
		className: 'tw-flex tw-flex-nowrap tw-pd-05'
	}, renderBasic(chat, null));
	...
	load(chat, rich, body, card, link).catch(err => {
...
async function load(chat, rich, body, card, link) {
	let data;
	try {
		...
		data = {error: err?.message === 'timeout' ? 'Timed out.' : String(err)};
	}
	...
	if ( ! doc ) {
		setChildren(body, renderBasic(chat, data));
		return;
	}

	const ctx = {
		vue: false,
		tList: (...args) => chat.i18n.tList(...args),
		i18n: chat.i18n,
...
function renderBasic(chat, data) {
	let title, description;

	if ( ! data )
		description = chat.i18n.t('card.loading', 'Loading...');
	else {
		title = data.title;
	...
	if ( ! title && ! description )
		description = chat.i18n.t('card.empty', 'No data was returned.');
