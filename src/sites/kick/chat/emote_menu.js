...
export default class EmoteMenu extends Module {
	...
	onEnable() {
		...
		this.on('..input:update-emotes', this.refresh, this);
	...
	open() {
		const button = document.querySelector('.ffz--kick-emote-button');
		if ( ! button )
			return;
		...
		this._outside = new ClickOutside(this.menu, event => {
			if ( ! event.target?.closest?.('.ffz--kick-emote-button') )
				this.close();
		});
			...
			sections.push(createElement('section', {className: 'ffz--kick-emote-menu__section'}, [
				...
				createElement('div', {className: 'ffz--kick-emote-menu__grid'}, emotes.map(emote =>
					createElement('button', {
						...
						type: 'button',
						title: emote.name,
						onClick: event => this.pick(emote, event)
